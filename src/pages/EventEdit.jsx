...
const EventEdit = () => {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            const response = await axios.put(`${server_uri}/events/${id}`, {
                name,
                description,
                date,
                time,
            ...
            navigate(`/campuspulse/event-details/${id}`);
    ...
    const getEventDetails = async () => {
        const response = await axios.get(`${server_uri}/events/${id}`)
        const EventData = response.data

        setName(EventData.name)
        setDescription(EventData.description)
        setDate(new Date(EventData.date).toISOString().slice(0, 10))
        setTime(EventData.time)
        setVenue(EventData.venue)
        setOrganizedBy(EventData.organizedBy)
        setPrice(EventData.price)
        setTotalSeats(EventData.totalSeats)
        setAvailableSeats(EventData.availableSeats)
        setRegistrationDeadline(new Date(EventData.registrationDeadline).toISOString().slice(0, 10))
            ...
            {token
                ? <>
                    <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
                        <FormHeading heading="Update an Event" />
                        ...
                        <div className="mt-5 sm:mx-auto sm:w-[1080px]">
                            <form onSubmit={submitHandler} method="POST" className="space-y-6">
                                ...
                                <TextArea label="Description" value={description} rows={10} placeholder='Event Description' setValue={setDescription} id="e_desc" name="description" type="textarea" />
                                ...
                                <Button type="submit" value="Update Event" />
                            </form>
                        </div>
                    </div>
                </>
